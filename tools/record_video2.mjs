 ...
 * 對時是算出來的，不是對出來的。
 *
 * 輸出：out/rubik-corners-first.mp4
 */
...
const VOICE = path.join(OUT, "voice2");

const arg = (n, d) => { const i = process.argv.indexOf(`--${n}`); return i > -1 ? process.argv[i + 1] : d; };
const FPS = Number(arg("fps", 30));
const QUALITY = Number(arg("quality", 0.92));
const URL_ = arg("url", "http://localhost:8051/web/record2.html");
const LIMIT = Number(arg("limit", 0));  // 只錄前 N 格，用來快速試版面
const SILENT = path.join(OUT, "silent2.mp4");
const FINAL = path.join(OUT, arg("out", "rubik-corners-first.mp4"));
...
async function main() {
  ...
  writeFileSync(path.join(OUT, "plan2.json"), JSON.stringify(plan, null, 2));
  console.log(`總長 ${plan.durationSec}s · ${plan.totalFrames} frames @ ${plan.fps}fps`);
  for (const s of plan.scenes) {
    const extra = s.totalSteps
      ? `  ${s.totalSteps} 步 ≈${(s.stepsPerFrame * FPS).toFixed(1)} 步/秒` : "";
